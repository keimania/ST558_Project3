...
## 6.3. Preparing for API

We calculate Mean for Numeric and Mode for Factors to use as default values for API.

```{r}
predictors <- c("BMI", "HighBP", "HighChol", "GenHlth", "Age", "PhysActivity", "DiffWalk", "HeartDiseaseorAttack")

# Calculate Mean for Numeric and Mode for Factors
defaults <- diabetes |>
  select(all_of(predictors)) |>
  summarise(
    BMI = mean(BMI, na.rm = TRUE),
    across(-BMI, \(x) as.numeric(names(sort(table(x), decreasing = TRUE))[1]))
  )
